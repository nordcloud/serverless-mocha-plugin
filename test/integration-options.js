...
describe('integration with options in stage prod', () => {
  before(() => {
    ...
    execSync(`ln -s ${path.join(process.env.MOCHA_PLUGIN_TEST_DIR, '/../node_modules')} ${tmpDir}/`);
  });

  it('should contain test params in cli info', () => {
    const test = execSync(`${serverlessExec} -h`);
  ...
  it('should run tests successfully against built package', () => {
    ...
    const test = execSync(`${serverlessExec} invoke test --stage prod --root testBuild/service`);
    const result = test.toString();

    expect(result).to.have.string(
      'Run tests against code under \'testBuild/service\'',
    );
    expect(result).to.have.string(
      'goodbye\n    ✓ implement tests here',
    );
